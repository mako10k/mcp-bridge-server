...
const PID_DIR = '/tmp';
const LOG_DIR = '/tmp';
const PID_FILE = join(PID_DIR, 'mcp-bridge-admin-ui.pid');
const LOG_FILE = join(LOG_DIR, 'mcp-bridge-admin-ui.log');
const SERVER_SCRIPT = join(__dirname, 'node_modules', 'vite', 'bin', 'vite.js');
// __dirname is natively available in CommonJS, no need to redefine
...
function startDaemon() {
  ...
  const child = spawn('node', [SERVER_SCRIPT, 'preview'], {
    detached: true,
  ...
  child.unref();
  console.log(`Admin UI daemon started with PID: ${child.pid}`);
